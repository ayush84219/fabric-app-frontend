...
import {
  ...
  Box, Hourglass, ArrowLeftRight, ExternalLink, History
} from 'lucide-react';
...
const DyeingMaterialForm = () => {
  ...
  const loadHistoryRolls = async () => {
    try {
      setIsLoadingHistory(true);
      const data = await store.getDyeingMaterials();
      setHistoryRolls(data || []);
    } catch (err) {
      console.error('Failed to load received rolls history:', err);
  ...
  const handleReprintRoll = (dm) => {
    const stickerData = {
      cmfName: dm.cmfName || 'Dyeing CMF',
      fabricName: dm.fabricName || '',
      group: dm.group || '',
      shade: dm.shade || '',
      weight: String(dm.weight),
      lotNumber: dm.lotNumber || '',
      billNumber: dm.billNumber || '',
      date: dm.date || '',
      location: dm.location || '',
      receivedPerson: dm.receivedPerson || '',
      authorizedPerson: dm.authorizedPerson || '',
      rollNumber: dm.rollNumber || 1,
      totalRolls: dm.batchTotal || 1,
      uniqueBarcodeId: dm.barcodeId,
      generatedAt: dm.generatedAt || '',
      timestamp: dm.timestamp || '',
      status: dm.status || 'in_stock'
    };

    if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN && printServiceStatus === 'ready') {
      printViaPythonService(stickerData);
      showNotification(`✓ Reprint job sent for Barcode: ${dm.barcodeId}`, 'success');
    } else {
      showNotification('⚠️ Print service offline. Unable to reprint.', 'error');
    }
  };
  ...
  const storeDataInGoogleSheets = async (data, rollNumber) => {
    ...
    try {
      const API_URL = 'https://fabric-backend-3.onrender.com//api/google-sheets/store-dyeing-data';
          ...
          <div className="modal" style={{ maxWidth: '850px', width: '95%' }}>
            <div className="modal-header">
              <div className="modal-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <History size={18} /> Dyeing Barcode Print History
              </div>
              <button className="modal-close-btn" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }} onClick={() => setShowHistoryModal(false)}>
                <X size={18} />
              </button>
            </div>
            <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <input
                  type="text"
                  className="form-control"
                  placeholder="Search by Lot Number, Barcode, or Fabric..."
                  value={historySearchQuery}
                  onChange={e => setHistorySearchQuery(e.target.value)}
                  style={{ flex: 1 }}
                />
                <a
                  href={store.downloadDyeingMaterialsFileUrl()}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="btn-page-switch"
                  style={{
                ...
                >
                  <ExternalLink size={14} /> Download Raw File
                </a>
              </div>
                ...
                {isLoadingHistory ? (
                  ...
                  <table>
                    ...
                    <tbody>
                      {historyRolls
                        .filter(r =>
                          String(r.lotNumber).toLowerCase().includes(historySearchQuery.toLowerCase()) ||
                          String(r.barcodeId).toLowerCase().includes(historySearchQuery.toLowerCase()) ||
                          String(r.fabricName).toLowerCase().includes(historySearchQuery.toLowerCase())
                        )
                        .map((r) => (
                          <tr key={r.id}>
                            <td style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                              {r.date} {r.generatedAt}
                            </td>
                            <td style={{ fontWeight: 700, color: 'var(--primary)' }}>{r.barcodeId}</td>
                            <td style={{ fontWeight: 600 }}>{r.lotNumber}</td>
                            <td style={{ fontSize: 12 }}>{r.fabricName}</td>
                            <td>{r.rollNumber} / {r.batchTotal}</td>
                            <td style={{ fontWeight: 700 }}>{r.weight} KG</td>
                            <td style={{ fontSize: 12 }}>{r.shade}</td>
                            <td>
                              {r.crossCheckStatus === 'verified' && (
                                <span style={{
                                  display: 'inline-flex',
                                  alignItems: 'center',
                                  gap: 4,
                              ...
                              {r.crossCheckStatus === 'missing_in_file' && (
                                <span style={{
                                  display: 'inline-flex',
                                  alignItems: 'center',
                                  gap: 4,
                                ...
                                }} title="This barcode is only in DB, missing from backup JSON file">
                                  ⚠️ DB Only
                                </span>
                              )}
                              {r.crossCheckStatus === 'missing_in_db' && (
                                <span style={{
                                  display: 'inline-flex',
                                  alignItems: 'center',
                                  gap: 4,
                                ...
                                }} title="This barcode is only in backup JSON file, missing from DB">
                                  ⚠️ File Only
                                </span>
                              )}
                              {r.crossCheckStatus === 'mismatch' && (
                                <span style={{
                                  display: 'inline-flex',
                                  alignItems: 'center',
                                  gap: 4,
                                ...
                                }} title="Attributes do not match between DB and backup JSON file">
                                  ⚠️ Mismatch
                                </span>
                              )}
                            </td>
                      ...
                      {historyRolls.length === 0 && (
                        <tr>
                          <td colSpan="9" style={{ textAlign: 'center', padding: 20, color: 'var(--text-muted)' }}>
                            No print records found in database or backup file.
                          </td>
                        </tr>
                      )}
                    </tbody>
